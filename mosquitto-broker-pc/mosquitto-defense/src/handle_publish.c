...
int handle__publish(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	if(qos > 0){//维护代理中所有store消息的列表
		db__message_store_find(context, mid, &stored);
	}
	if(!stored){
		dup = 0;
		//在这里存入发送方的context，但是要考察一下context是如何管理的，只保存指针如何避免相关的东西被释放。
		if(db__message_store(db, context->id, mid, topic, qos, payloadlen, &payload, retain, &stored, 0, context)){
			return 1;
		}
	}else{
		mosquitto__free(topic);
...
process_bad_message:
	...
	switch(qos){
		...
		case 2:
			db__message_store_find(context, mid, &stored);
			if(!stored){
				if(db__message_store(db, context->id, mid, NULL, qos, 0, NULL, false, &stored, 0, NULL)){
					return 1;
				}
