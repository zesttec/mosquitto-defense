...
int handle__connect(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	if(will){
		...
		//在connect的时候检查了will_topic的格式检查保证没有+#，不是权限检查
	...
	if(context->listener && context->listener->ssl_ctx && (context->listener->use_identity_as_username || context->listener->use_subject_as_username)){
		...
		if(!username_flag){//没有用户名检查设置是否允许匿名
...
#endif
	//很棒，提供了选项将clientID强制设置为username，同时也提供了将username强制设置成证书CN，这样从mqtt角度认证看似完美了
	...
	if(found_context){//冲突就让之前的下线
		...
		//开始恢复会话，代码可见恢复会话只根据clientID，如果攻击者可以伪造clientID就可以恢复别人的会话。不过恢复会话了也没啥用，只能绕过sub权限检查， 消息被发送时会检查receive权限。
		...
		found_context->clean_session = true;//这是为了释放掉之前的context专门设置的
	...
	//尽管前面处理的早，这里才把last will存入contex里，所以前面的disconnect不会触发last will消息。实现中在放入时没有检查acl，只是发送时进行了检查。
	if(will_struct){
	...
	//很棒，在连接成功后，立刻检查一下现在这个客户端context之前剩余的message是否符合ACL。
