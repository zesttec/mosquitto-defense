...
static int subs__process(struct mosquitto_db *db, struct mosquitto__subhier *hier, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store *stored, bool set_retain)
{
	...
	if(retain && set_retain){//如果是一个retained消息
		...
		if(stored->payloadlen){//在当前主题结点中保存这个retained消息
...
//订阅后，如果确实该主题上之前有retained消息，将该消息加入客户端队列
static int retain__process(struct mosquitto_db *db, struct mosquitto_msg_store *retained, struct mosquitto *context, const char *sub, int sub_qos)
{
	...
	//！！！应该在这里也检查之前的retain消息有没有权限发给客户端！！！没有的话就将该消息清除
	//那么这个保存消息的结构就需要加入一个存放之前context的变量
	if (retained->source_context) {
		//retained->source_context->id = retained->source_id;
		rc = mosquitto_acl_check(db, retained->source_context, retained->topic, retained->payloadlen, UHPA_ACCESS(retained->payload, retained->payloadlen),
				retained->qos, retained->retain, MOSQ_ACL_WRITE);
		if(rc == MOSQ_ERR_ACL_DENIED){
			printf("---source id = %s\n\n", retained->source_id);
			printf("Retained Message Attack is Detected!!!\n");
			return MOSQ_ERR_SUCCESS;
		}else if(rc != MOSQ_ERR_SUCCESS){
			return rc;
		}
	} else {
		printf("Retained Message Source is NULL!!!\n");
	}
