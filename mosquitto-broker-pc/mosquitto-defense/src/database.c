...
void db__msg_store_remove(struct mosquitto_db *db, struct mosquitto_msg_store *store)
{
	...
	//在保存的消息消亡时，记得减少源的保存消息的计数
	if (store->source_context) {//可能不是只有retain消息用到了保存消息？
		store->source_context->retained_message_count--;
		//在这个retained message消亡的时候检查一下是不是可以释放这个对应源的context
		if (store->source_context->retained_message_count <= 0 && 
			store->source_context->clean_session == true &&
			store->source_context->state == mosq_cs_disconnected) {
				printf("Delete context because of Retained Message is deleted!");
				//context__add_to_disused(db, store->source_context);
			}
	} else {
		//printf("This message doesn't have source_context!\n");
	}
...
int db__messages_easy_queue(struct mosquitto_db *db, struct mosquitto *context, const char *topic, int qos, uint32_t payloadlen, const void *payload, int retain)
{
	...
	if(db__message_store(db, source_id, 0, topic_heap, qos, payloadlen, &payload_uhpa, retain, &stored, 0, NULL)) return 1;

	return sub__messages_queue(db, source_id, topic_heap, qos, retain, &stored);
}
//这个函数将消息的各种属性存入stored保存下来。所以要修改这个函数，保存之前发送者context的权限。
...
int db__message_store(struct mosquitto_db *db, const char *source, uint16_t source_mid, char *topic, int qos, uint32_t payloadlen, mosquitto__payload_uhpa *payload, int retain, struct mosquitto_msg_store **stored, dbid_t store_id, struct mosquitto *source_context)
{
	...
	//在这里存入消息源的context
	if (source_context && retain) {
		temp->source_context = source_context;
		temp->source_context->retained_message_count++;
		printf("The source_context of this Retained Message is Stored !!!\n");
	} else {
	//	printf("The source_context of this Retained Message is NULL !!!\n");
		temp->source_context = NULL;
	}
	//end
...
//这个函数似乎是给众客户端即context转发消息（从inflight_msgs中取出然后发送）
int db__message_write(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	tail = context->inflight_msgs;//取出将要发给客户端的消息们
	while(tail){
		msg_count++;
		mid = tail->mid;
		retries = tail->dup;
		retain = tail->retain;//每一个消息是个数据结构，甚至都有retain标志
		topic = tail->store->topic;
		qos = tail->qos;
		payloadlen = tail->store->payloadlen;
		payload = UHPA_ACCESS_PAYLOAD(tail->store);

		switch(tail->state){//根据消息的状态发送消息
			case mosq_ms_publish_qos0:
				//放入这个send__publish后，包就进入发包队列等着被发走了，没有检查。是在消息进入客户端context队列前检查的。
