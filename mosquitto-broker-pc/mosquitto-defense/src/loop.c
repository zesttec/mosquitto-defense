...
void do_disconnect(struct mosquitto_db *db, struct mosquitto *context)
{
...
#endif
	{
...
#ifdef WITH_BRIDGE
		if(context->clean_session && !context->bridge && context->retained_message_count == 0){
#else
		//在这里加入判断，如果这个客户端注册过retained message，那么就不释放，不进入这个if；当没有retain后，再让释放，因此需要计数有多少个retain消息。
		if(context->clean_session && context->retained_message_count == 0){
#endif
			//在这里别让断开连接的时候清掉context，但是client_id还是可以删掉的吧？No!
			//printf("retained_message_count is %d \n\n", context->retained_message_count);
		...
		context->state = mosq_cs_disconnected;//设置为已断开状态
	}
}
...
#endif
{
	...
	HASH_ITER(hh_sock, db->contexts_by_sock, context, ctxt_tmp){
		...
		if(pollfds[context->pollfd_index].revents & POLLOUT){
			...
			if(packet__write(context)){//如果是可写事件就发送所有数据包
				do_disconnect(db, context);
				continue;
			}
		}
...
#endif
			do{
				if(packet__read(db, context)){//如果是读事件就读数据包
					do_disconnect(db, context);
					continue;
				}
