...
int packet__write(struct mosquitto *mosq)
{
	...
	while(mosq->current_out_packet){//循环把current_out_packet都发出去
...
#endif
{
	 ...
	 * available - ie. at least one bytewe.  What  do depends on what data we
	 * already have.
	...
	//至此读取数据包完毕，并且提前检查过了第一个数据包是不是connect
