...
options=dict()
brokers=["156.56.159.126", "156.56.159.126","192.168.1.157","192.168.1.204","192.168.1.185","test.mosquitto.org",\
...
options["interval"]=0.5 #loop time when sensor publishes in verbose
options["interval_pub"]=10 # in non chatty mode publish
...
chatty=False
interval=1 #loop time when sensor publishes
sensor_pub_interval=10# how often to publish if status is unchanged
##
...
def publish_status(client):
    ...
    if client.sensor_status_old!=client.sensor_status or pubflag:
        ...
        client.publish(sensor_status_topic, str(time.time() ),0,True)
        print("publishing time to topic", sensor_status_topic)
        print("publish on",sensor_status_topic,\
              " message  ",str(time.time() )) #1000000
...
def Initialise_client_object():
    mqtt.Client.last_pub_time= time.time() ####time to str
    mqtt.Client.topic_ack=[]
    mqtt.Client.run_flag=True
    mqtt.Client.subscribe_flag=False
    mqtt.Client.sensor_status= time.time()#########timestamp float.
    #states[1]
...
sensor_status_topic= "sensors/device_10"#options["topic_base"]+"/"+cname
